...
Servo::Servo(hstring url, hstring args, GLsizei width, GLsizei height,
  ...
  for (auto pref : prefs.Values()) {
    auto key = *hstring2char(pref.Key()); // FIXME: is the unallocated?
    ...
    if (type == Windows::Foundation::PropertyType::Boolean) {
      ...
      pref.value = *hstring2char(unbox_value<hstring>(value)); // FIXME: free?
  ...
  capi::CPrefList pref_list = {cprefs.size(), cprefs.data()};
...
// FIXME: why not char*? Does freeing the unique pointer magically free the
// char*?
