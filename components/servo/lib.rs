...
impl<Window> Servo<Window> where Window: WindowMethods + 'static {
    ...
    fn handle_window_event(&mut self, event: WindowEvent) {
        match event {
            ...
            WindowEvent::Resize => {
                // FIXME: only compositor is updated
                // Constellation needs update too. It gets a new window_size,
                // but it's not only what we want. Maybe it should be replaced
                // with coordinates.
    ...
    fn receive_messages(&mut self) {
        while let Some(msg) = self.embedder_receiver.try_recv_embedder_msg() {
            match (msg, self.compositor.shutdown_state) {
                ...
                // (EmbedderMsg::MoveTo(top_level_browsing_context, point), ShutdownState::NotShuttingDown) => {
                //     self.compositor.window.set_position(top_level_browsing_context, point);
                // },

                // (EmbedderMsg::ResizeTo(top_level_browsing_context, size), ShutdownState::NotShuttingDown) => {
                //     self.compositor.window.set_inner_size(top_level_browsing_context, size);
                // },

                // (EmbedderMsg::AllowNavigation(top_level_browsing_context, url, response_chan), ShutdownState::NotShuttingDown) => {
                ...
                // },
                // (EmbedderMsg::HistoryChanged(top_level_browsing_context, entries, current), ShutdownState::NotShuttingDown) => {
                //     self.compositor.window.history_changed(top_level_browsing_context, entries, current);
                // },
                // (EmbedderMsg::SetFullscreenState(top_level_browsing_context, state), ShutdownState::NotShuttingDown) => {
                ...
                // },
                // (EmbedderMsg::Panic(top_level_browsing_context, reason, backtrace), ShutdownState::NotShuttingDown) => {
