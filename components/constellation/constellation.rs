...
impl Log for FromScriptLogger {
    ...
    fn log(&self, record: &LogRecord) {
        if let Some(entry) = log_entry(record) {
            ...
            // FIXME: should be pipeline_id, not top_level_id
          ...
          STF: ScriptThreadFactory<Message=Message>
{
    ...
    fn handle_request_from_script(&mut self, message: (PipelineId, FromScriptMsg)) {
        ...
        match content {
            ...
            FromScriptMsg::LoadUrl(load_data, replace) => {
                ...
                self.handle_load_url_msg(source_pipeline_id, load_data, replace);
            }
            ...
            FromScriptMsg::JointSessionHistoryLength(sender) => {
                // FIXME: top_level_browsing_context_id can be guess here.
            ...
            FromScriptMsg::RemoveIFrame(browsing_context_id, sender) => {
                // FIXME: browsing_context_id can be guess here
            ...
            FromScriptMsg::Alert(message, sender) => {
                ...
                self.handle_alert(source_top_ctx_id, source_pipeline_id, message, sender);
            }
            ...
            FromScriptMsg::LogEntry(top_level_browsing_context_id, thread_name, entry) => {
                // FIXME(paul): why can't we use source_top_ctx_id?
                self.handle_log_entry(top_level_browsing_context_id, thread_name, entry);
            }
    ...
    fn handle_alert(&mut self,
        ...
        // FIXME(paul): do we want to use pipeline_id or top_level_browsing_context_id.pipeline_id???
    ...
    fn load_url(&mut self, source_id: PipelineId, load_data: LoadData, replace: bool) -> Option<PipelineId> {
        ...
        // FIXME(paul): to properly send AllowNavigation, we need the top level browsing context, which we don't always have.
        // FIXME: this is important. This required the emebdder to send events with TLBC id.
    ...
    fn handle_load_start_msg(&mut self, top_level_browsing_context_id: TopLevelBrowsingContextId, _pipeline_id: PipelineId) {
        // FIXME(paul): only if pipeline is the top level one
