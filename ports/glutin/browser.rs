...
use servo::embedder_traits::{EmbedderMsg, FilterPattern, PromptDefinition, PromptResult};
                                ...
                                match definition {
                                    ...
                                    PromptDefinition::YesNo(message, sender) => {
                                        let result = tinyfiledialogs::message_box_yes_no(
                                            "", &tiny_dialog_escape(&message), MessageBoxIcon::Warning, YesNo::No,
                                        );
                                    ...
                                    PromptDefinition::OkCancel(message, sender) => {
                                        let result = tinyfiledialogs::message_box_ok_cancel(
                                            "", &tiny_dialog_escape(&message), MessageBoxIcon::Warning, OkCancel::Cancel,
                                        );
                                        sender.send(match result {
                                    ...
                                    PromptDefinition::Input(message, default, sender) => {
                                        let result = tinyfiledialogs::input_box("", &tiny_dialog_escape(&message), &tiny_dialog_escape(&default));
                                        sender.send(result)
                                    }
                                }
                            })
