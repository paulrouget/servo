...
// PAUL: TopLevelBrowsingContextId is used to reference one browser instance
...
use std::rc::Rc;
use std::thread;
use servo::compositing::windowing::WindowNavigateMsg;
...
struct Tabs {
    pub servo: Servo<app::window::Window>,
...
struct Tab {
    ...
    ctx_id: TopLevelBrowsingContextId,
}
...
impl Tabs {
    pub fn new(window: Rc<app::window::Window>, initial_load_data: Vec<LoadData>) -> Tabs {
        ...
        for load_data in initial_load_data {
            // Create a new ctx_id + pipeline
            let id = servo.new_browser(load_data, None).unwrap();
            browsers.push(Tab {ctx_id: id, url: None});
        }
        let tabs = Tabs {
            servo: servo,
            browsers: browsers,
    ...
    pub fn add_and_select(&mut self, load_data: LoadData, context: Option<String>) {
        ...
        self.browsers.push(Tab {ctx_id: id, url: None});
    ...
    fn active_selected_browser(&self) {
        // PAUL: this is where we tell WR to display this specific ctx_id tree
        self.servo.set_visible_browser(self.browsers[self.selected].ctx_id);
    }
...
fn main() {
    ...
    let mut tabs = Tabs::new(
        window.clone(),
        vec![ LoadData::new(ServoUrl::parse("http://example.com").unwrap(), None, None, None) ]
    );
    ...
    loop {
        ...
        for event in window_events {
            match event {
                ...
                // Open new tab.
                // This won't work because the constellation only support on top level ctx_id at the moment.
                WindowEvent::KeyEvent(Some('t'), _, KeyState::Pressed, CMD_OR_CONTROL) => {
                    println!("cmd-t");
                    let load_data = LoadData::new(new_tab_url.clone(), None, None, None);
                ...
                WindowEvent::KeyEvent(Some('l'), _, KeyState::Pressed, CMD_OR_CONTROL) => {
                    ...
                    if let Some(request) = tinyfiledialogs::input_box("Enter url", "Url:", "") {
						...
						match url {
							Ok(url) => {
                                events.push(WindowEvent::LoadUrl(tabs.active_tab().ctx_id, url.to_string()));
							},
                ...
                WindowEvent::KeyEvent(Some('r'), _, KeyState::Pressed, SUPER) => {
                    println!("cmd-r");
                    events.push(WindowEvent::Reload(tabs.active_tab().ctx_id));
                },
                ...
                // Go back
                WindowEvent::KeyEvent(_, Key::Left, KeyState::Pressed, SUPER) => {
                    println!("cmd-left");
                    events.push(WindowEvent::Navigation(tabs.active_tab().ctx_id, WindowNavigateMsg::Back));
                },

                // Go forward
                WindowEvent::KeyEvent(_, Key::Right, KeyState::Pressed, SUPER) => {
                    println!("cmd-left");
                    events.push(WindowEvent::Navigation(tabs.active_tab().ctx_id, WindowNavigateMsg::Forward));
                },
        ...
        for event in servo_events {
            match event {
                ...
                BrowserEvent::SetUrl(ctx_id, url) => {
                    tabs.browsers.iter_mut().find(|ref b| b.ctx_id == ctx_id).unwrap().url = Some(url);
                    tabs.print_tabs();
				},


                // User Ctrl-clicked on link. Open link in a new browser.
                BrowserEvent::HandleUrl(url) => {
